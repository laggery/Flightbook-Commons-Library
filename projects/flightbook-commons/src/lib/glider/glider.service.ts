...
export class GliderService extends Store<Glider[]> {
  ...
  postGlider(glider: Glider): Observable<Glider> {
    return this.http.post<Glider>(`${this.environment.baseUrl}/gliders`, glider).pipe(
      map((response: Glider) => {
        this.setState([]);
  ...
  putGlider(glider: Glider): Observable<Glider> {
    return this.http.put<Glider>(`${this.environment.baseUrl}/gliders/${glider.id}`, glider).pipe(
      map((response: Glider) => {
  ...
  deleteGlider(glider: Glider): Observable<Glider> {
    return this.http.delete<Glider>(`${this.environment.baseUrl}/gliders/${glider.id}`).pipe(
      map((response: any) => {
        const list = this.getValue();
